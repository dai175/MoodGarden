...
- [ ] 1. **MoodType enum** — `MoodGarden/Models/MoodType.swift` (新規)
...
- [ ] 2. **MoodEntry モデル** — `MoodGarden/Models/MoodEntry.swift` (新規)
...
- [ ] 3. **MonthlyGarden モデル** — `MoodGarden/Models/MonthlyGarden.swift` (新規)
  - `@Model final class MonthlyGarden` — id, year, month, snapshotImage, completedAt

- [ ] 4. **AppState** — `MoodGarden/App/AppState.swift` (新規)
  - `@Observable class AppState` — hasCompletedOnboarding (`@AppStorage`)

- [ ] 5. **GardenViewModel** — `MoodGarden/ViewModels/GardenViewModel.swift` (新規)
...
## フェーズ 2: コア UI (SwiftUI) [ ]

ムード記録の一連のユーザーフローを SwiftUI で完成。SpriteKit の代わりに仮グリッド表示。

### タスク

- [ ] 1. **DesignConstants** — `MoodGarden/Views/Theme/DesignConstants.swift` (新規)
  - 色定数: background `#0A1A12`~`#0D2818`, text `#E8E4DC` (0.8), accent `#1D9E75`
  - アニメーション duration, フォント設定

- [ ] 2. **GardenView** — `MoodGarden/Views/GardenView.swift` (新規)
  - フルスクリーン。上部: 月名、右上: 歯車アイコン、下部: MoodSelector
  - フェーズ 2 では LazyVGrid で色付き丸の仮表示

- [ ] 3. **MoodSelectorView + MoodIcon** — `MoodGarden/Views/MoodSelectorView.swift`, `MoodGarden/Views/Components/MoodIcon.swift` (新規)
  - 折りたたみ/展開アニメーション (0.8s ease-in-out)
  - hasTodayEntry == true で非表示

- [ ] 4. **RootView** — `MoodGarden/Views/RootView.swift` (新規)
...
- [ ] 1. **GardenGridLayout** — `MoodGarden/Garden/GardenGridLayout.swift` (新規)
  - 7列 x 5行。日番号 → (column, row) → CGPoint 変換
  - テスト: `MoodGardenTests/GardenGridLayoutTests.swift`

- [ ] 2. **GardenElement protocol** — `MoodGarden/Garden/Elements/GardenElement.swift` (新規)
  - `func createNode(seed: Int, cellSize: CGSize) -> SKNode`

- [ ] 3. **7 Elements** (各 新規) — `MoodGarden/Garden/Elements/`
...
- [ ] 4. **GardenRenderer** — `MoodGarden/Garden/GardenRenderer.swift` (新規)
  - `[MoodEntry]` → SKNode 配列。Element ファクトリ呼び出し
  - テスト: `MoodGardenTests/GardenRendererTests.swift`

- [ ] 5. **GardenScene** — `MoodGarden/Garden/GardenScene.swift` (新規)
  - `SKScene` サブクラス。configure(with:), addEntry(_:animated:)
  - 背景色 `#0A1A12`、scaleMode `.resizeFill`

- [ ] 6. **GardenView SpriteKit 統合** — `MoodGarden/Views/GardenView.swift` (修正)
